...
filesRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      res.status(502).json({
        error: `Cloudflare R2 storage error: ${
...
filesRouter.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      const content = await r2Storage.getFileContent(
        server.userId,
        server.id,
        filePath
      );
...
// Upload file (buffer or base64) to R2
filesRouter.post(
  '/:id/files/upload',
  requirePermission('files.write'),
  async (req: AuthenticatedRequest, res) => {
    ...
    // Allow empty files, but path and content field must exist.
    if (
      !path ||
      content === undefined ||
      content === null
      ...
      res.status(400).json({
        error: 'Path and content payload are required.',
      });
      return;
    }

    try {
      // Convert incoming content into Buffer.
      ...
      if (shouldExtract) {
        ...
        const result = await r2Storage.extractZip(
          server.userId,
          server.id,
          buffer,
          targetDir
        ...
        res.json({
          success: true,
          filesExtracted: result.filesCount,
        });

        return;
      }

      // Normal file upload to Cloudflare R2.
      await r2Storage.putFileContent(
        server.userId,
        server.id,
        path,
...
// Delete file or directory in R2
filesRouter.delete(
  '/:id/files',
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    if (!path) {
      res.status(400).json({
        error: 'File/directory path is required.',
      });
      return;
    }
...
filesRouter.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      res.status(500).json({
        error: `Failed to create folder in R2: ${
          err?.message || 'Unknown storage error'
        }`,
      });
    }
...
// Extract an existing zip in R2
filesRouter.post(
  '/:id/files/extract',
  requirePermission('files.extract'),
  async (req: AuthenticatedRequest, res) => {
    const server = checkServerAccess(req, res);
    if (!server) return;
    ...
    try {
      const { client, bucket } =
        (r2Storage as any).getClient();

      const key =
        `users/${server.userId}/servers/${server.id}/files/` +
        `${zipPath.replace(/^\/+/, '')}`;
...
// Compress files in R2 to ZIP
filesRouter.post(
  '/:id/files/compress',
  requirePermission('files.compress'),
  async (req: AuthenticatedRequest, res) => {
    const server = checkServerAccess(req, res);
    if (!server) return;
    ...
    const {
      folderPath = '',
      outputZipName = 'archive.zip',
    } = req.body;

    try {
      const zipBuffer =
        await r2Storage.compressZip(
          server.userId,
          server.id,
          folderPath
        );
      ...
      logActivity({
        ...
        details:
          `Compressed folder '${folderPath}' ` +
          `to '${outputZipName}' in R2`,
        ip: req.ip,
      });
...
// Download file or presigned URL
filesRouter.get(
  '/:id/files/download',
  requirePermission('files.read'),
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      const presignedUrl =
        await r2Storage.getPresignedDownloadUrl(
          server.userId,
          server.id,
          filePath,
          300
...
// POST /api/v1/servers/:id/files/presigned-upload
// Presigned URL for direct large upload to R2
filesRouter.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      const uploadUrl =
        await r2Storage.getPresignedUploadUrl(
          server.userId,
          server.id,
          path,
          900
...
// POST /api/v1/servers/:id/files/sync-deploy
// Syncs R2 files into git bridge and triggers deploy on Render
filesRouter.post(
  '/:id/files/sync-deploy',
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      logActivity({
        ...
        details:
          `Synced R2 files to Git Bridge and ` +
          `triggered Render deploy ${deploy.id}`,
        ip: req.ip,
      });
      ...
      res.json({
        ...
        deploy,
        message:
          'Files synchronized from Cloudflare R2. ' +
          'Deployment initiated on Render.',
      });
    } catch (err: any) {
      res.status(500).json({
        error:
          `Sync and deploy failed: ${
            err?.message || 'Unknown deployment error'
          }`,
      });
    }
  }
