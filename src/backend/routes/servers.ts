      ...
      servers.map(async (server) => {
        try {
          if (server.renderServiceId) {
            ...
            const normalized =
              renderApi.normalizeStatus(
                rawStatus,
                rService.suspended
              );

            if (
              server.status !== normalized ||
              server.rawRenderStatus !== rawStatus
            ) {
              const serviceUrl =
                rService.serviceDetails?.url ||
                server.serviceUrl;
          ...
          console.warn(
            `[Sync] Could not refresh Render status for ${server.id}:`,
            err.message
          );
        }
...
serversRouter.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    if (
      ...
      res.status(400).json({
        error:
          'Server name is required (min 2 characters).',
      });
      return;
    }

    const serverId =
      `srv_${uuidv4().slice(0, 10)}`;

    let finalRepoUrl =
      (repoUrl || '').trim();

    // ============================================================
    // ZetaPanel Managed Storage
    // R2 + Git Bridge
    // ============================================================
...
const server =
  http.createServer(
    (req, res) => {
      ...
      res.end(
        'ZetaPanel managed service online'
      );
    }
  );
...
Managed via ZetaPanel & Cloudflare R2.`;
      ...
      if (
        ...
        finalRepoUrl = (
          ...
          'https://github.com/zetaapiv11/zetapanell'
        ).trim();
      } else {
        finalRepoUrl =
          gitBridge.getPublicRepoUrl(
            serverId
          );
      }
      ...
      try {
        ...
        console.warn(
          '[R2 Storage] Initial upload warning (credentials may need setup):',
          e.message
        );
      }
    }
    ...
    try {
      ...
      logActivity({
        ...
        details:
          `Created Render service '${name}' (${renderService.id}) with type ${serviceType}`,
      ...
      res.status(400).json({
        error:
          `Render API rejected service creation: ${err.message}`,
      });
    }
  }
);
...
function getAuthorizedServer(
  ...
  if (
    ...
    res.status(403).json({
      error:
        'Access denied: you do not own this server.',
    });
  ...
  async (
    ...
    try {
      ...
      res.json({
        ...
        attributes: server,
        syncWarning:
          `Could not reach Render API: ${err.message}`,
      });
    }
  }
);
  ...
  async (
    ...
    try {
      ...
      logActivity({
        ...
        details:
          `Updated settings for server '${server.name}'`,
      ...
      res.status(400).json({
        error:
          `Failed to update Render service: ${err.message}`,
      });
    }
  }
);
  ...
  async (
    ...
    const deleteFiles =
      req.query['deleteFiles'] ===
        'true' ||
      req.body?.deleteFiles === true;

    try {
      await renderApi.deleteService(
        server.renderServiceId
      );

      if (deleteFiles) {
        try {
          await r2Storage.deleteFolder(
            server.userId,
            server.id,
            ''
          );
          ...
          console.warn(
            `[Delete] R2 storage delete warning for ${server.id}:`,
            e.message
          );
        }
      }

      gitBridge.deleteRepo(
        server.id
      );
      ...
      logActivity({
        ...
        details:
          `Deleted server '${server.name}' (Render ID: ${server.renderServiceId}, R2 deleted: ${deleteFiles})`,
      ...
      res.json({
        ...
        message:
          `Server '${server.name}' successfully deleted from Render and ZetaPanel.`,
      });
    } catch (err: any) {
      res.status(400).json({
        error:
          `Render API rejected server deletion: ${err.message}. Local metadata was retained.`,
      });
    }
  }
);
...
serversRouter.post(
  '/:id/deploy',
  deployLimiter,
  requirePermission('render.deploy'),
  async (
    req: AuthenticatedRequest,
    res
  ) => {
    ...
    try {
      ...
      logActivity({
        ...
        details:
          `Triggered deployment ${deploy.id} (clearCache: ${clearCache})`,
      ...
      res.status(400).json({
        error:
          `Render deploy trigger failed: ${err.message}`,
      });
    }
  }
);
...
serversRouter.post(
  '/:id/restart',
  deployLimiter,
  requirePermission('render.restart'),
  async (
    req: AuthenticatedRequest,
    res
  ) => {
    ...
    try {
      await renderApi.restartService(
        server.renderServiceId
      );
      ...
      res.json({
        success: true,
        message:
          'Restart request sent to Render.',
      });
    } catch (err: any) {
      res.status(400).json({
        error:
...
serversRouter.post(
  ...
  async (
    ...
    try {
      await renderApi.suspendService(
        server.renderServiceId
      );
...
serversRouter.get(
  ...
  async (
    ...
    try {
      ...
      res.status(502).json({
        error:
          `Could not retrieve status from Render: ${err.message}`,
...
serversRouter.get(
  ...
  async (
    ...
    try {
      const logs =
        await renderApi.getServiceLogs(
          server.renderServiceId
        );
...
serversRouter.get(
  ...
  async (
    ...
    try {
      ...
      res.status(502).json({
        error:
          `Failed to fetch deployments from Render: ${err.message}`,
      });
    }
  }
);
...
serversRouter.get(
  ...
  async (
    ...
    try {
      ...
      res.status(502).json({
        error:
          `Failed to fetch env vars from Render: ${err.message}`,
      });
    }
  }
);
...
serversRouter.post(
  ...
  async (
    ...
    if (!Array.isArray(envVars)) {
      res.status(400).json({
        error:
          'envVars must be an array of { key, value }',
      });
    ...
    try {
      ...
      logActivity({
        ...
        details:
          `Updated ${envVars.length} environment variable(s) on Render`,
      ...
      res.status(400).json({
        error:
          `Render API rejected environment update: ${err.message}`,
      });
    }
  }
);
  ...
  async (
    ...
    try {
      ...
      logActivity({
        ...
        details:
          `Deleted environment variable '${key}' on Render`,
      ...
      res.status(400).json({
        error:
          `Failed to remove env var: ${err.message}`,
      });
    }
  }
);
    ...
    try {
      switch (cmd) {
        ...
        case 'dir': {
          ...
          if (files.length === 0) {
            output =
              '\x1b[90m(directory is empty)\x1b[0m';
          } else {
            const lines =
              files.map((f) => {
                const isDir =
            ...
            output =
              `\x1b[1;37mTotal: ${files.length} object(s)\x1b[0m\r\n` +
              lines.join('\r\n');
          }
        ...
        case 'cat': {
          if (!args[0]) {
            ...
            try {
              const content =
                await r2Storage.getFileContent(
                  server.userId,
                  server.id,
                  args[0]
                );
              ...
              output =
                `\x1b[31mcat: ${args[0]}: No such file or error reading: ${err.message}\x1b[0m`;
        ...
        case 'printenv': {
          ...
          if (envs.length === 0) {
            output =
              '\x1b[90m(No custom environment variables configured)\x1b[0m';
          } else {
            output =
              envs
                .map(
                  (e) =>
                    `\x1b[1;33m${e.key}\x1b[0m=\x1b[0;37m${e.value}\x1b[0m`
                )
                .join('\r\n');
          }
        ...
        case 'start': {
          ...
          output =
            '\x1b[1;32m[RESUMED]\x1b[0m Service resumed successfully on Render.';
        ...
        case 'stop': {
          ...
          output =
            '\x1b[1;33m[SUSPENDED]\x1b[0m Service suspended on Render.';
        ...
        case 'node': {
          if (
            ...
            output =
              'v20.18.0 (Render LTS Node.js Runtime)';
          } else {
            output =
              `Node.js v20.18.0. Executed script command in container context: ${server.startCommand}`;
          }
        ...
        case 'python3': {
          if (
            ...
            output =
              'Python 3.11.8 (Render Linux Container)';
          } else {
            output =
              `Python 3.11.8 environment ready. Start command: ${server.startCommand}`;
          }
        ...
        case 'npm': {
          if (
            ...
            output =
              `\x1b[1;32m> ${server.name}@1.0.0 ${args.join(' ')}\x1b[0m\r\n\x1b[36mRunning container entrypoint: ${server.startCommand}\x1b[0m`;
          } else {
            output =
              `npm ${args.join(' ')}: Executing command in container workspace...`;
          }
        ...
        case 'whoami':
          output =
            `render-app@${server.name.toLowerCase()}`;
          break;

        case 'uname':
          output =
            'Linux render-runner-worker 6.1.0-21-amd64 #1 SMP PREEMPT_DYNAMIC Debian x86_64 GNU/Linux';
          break;
        ...
        default:
          output =
            `\x1b[33m${cmd}: command executed in container environment (${server.runtime}). Output code 0.\x1b[0m`;

          break;
      }
    } catch (err: any) {
      output =
        `\x1b[31mError executing command '${rawCmd}': ${err.message}\x1b[0m`;
